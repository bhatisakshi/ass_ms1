...
def convert_wav_to_mp3(input_folder, processing_folder):
    ...
    for root, dirs, files in os.walk(input_folder):
        for file_name in files:
            if file_name.endswith('.wav'):
                ...
                # Create a folder in the processing directory with the same name as the main folder
                processing_main_folder = os.path.join(processing_folder, main_folder_name)
                os.makedirs(processing_main_folder, exist_ok=True)

                # Get filename without extension
                filename_without_extension = os.path.splitext(file_name)[0]

                # Create a subfolder inside the main folder for original files
                original_folder = os.path.join(processing_main_folder, filename_without_extension, "original")
                ...
                converted_folder = os.path.join(processing_main_folder, filename_without_extension, "converted")
                ...
                chunks_folder = os.path.join(processing_main_folder, filename_without_extension, "chunks")
...
convert_wav_to_mp3(input_folder, processing_folder)
...
# Function to create an Excel report for SourceFile table
def create_source_file_report(database_name, report_folder):
    conn = sqlite3.connect(database_name)
    cursor = conn.cursor()

    # Update all file statuses to 'completed'
    cursor.execute("UPDATE SourceFile SET status = 'completed'")
...
def calculate_file_counts(database_name):
    ...
    cursor.execute("SELECT COUNT(*) FROM SourceFile WHERE status = 'processed'")
